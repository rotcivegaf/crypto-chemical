...
describe("CryptoChemical", () => {
  ...
  it("Function mintEnergy", async () => {
    ...
    const prevBal = await cryptoChemical["balanceOf(address)"](
      beneficiary.address
    );
    ...
    expect(
      await cryptoChemical["balanceOf(address)"](beneficiary.address)
  ...
  it("Function cancelSignHash", async () => {
    ...
    await expect(await cryptoChemical.cancelSignHash(msgHash)).to.emit(
      cryptoChemical,
      "CancelHash"
    );
  ...
  describe("Function signMintEnergy", () => {
    it("Mint energy with signature", async () => {
      ...
      const prevBeneficiaryAmount = await cryptoChemical["balanceOf(address)"](
        beneficiary.address
      );
    ...
    it("Try mint energy with expired signature", async () => {
      await expect(
        cryptoChemical
          .connect(beneficiary)
          .signMintEnergy(beneficiary.address, 1, 0, 1, [])
  ...
  describe("OnlyOwner functions", () => {
    it("setURI", async () => {
      await expect(
        cryptoChemical.connect(notOwner).setURI("")
      ).to.be.revertedWith("Ownable: caller is not the owner");
    });
    it("setManager", async () => {
      await expect(
        cryptoChemical.connect(notOwner).setManager(address0x)
      ).to.be.revertedWith("Ownable: caller is not the owner");
    });
    it("mintEnergy", async () => {
      await expect(
        cryptoChemical.connect(notOwner).mintEnergy(notOwner.address, 1)
      ).to.be.revertedWith("Ownable: caller is not the owner");
    });
    it("cancelSignHash", async () => {
      ...
      await expect(
        cryptoChemical.connect(notOwner).cancelSignHash(msgHash)
      ).to.be.revertedWith("Ownable: caller is not the owner");
    });
  });
  describe("Function setManager", () => {
    ...
    it("Try set a address(0) as manager", async () => {
      await expect(cryptoChemical.setManager(address0x)).to.be.revertedWith(
  ...
  describe("Function mint", () => {
    it("Mint random tokens", async () => {
      ...
      const prevBal = await cryptoChemical["balanceOf(address,uint256)"](
        beneficiary.address,
        id
      );
      ...
      await cryptoChemical
        .connect(manager)
        .mint(beneficiary.address, id, amount, []);
    ...
    it("Try mint without being the manager", async () => {
      await expect(
        cryptoChemical.connect(owner).mint(owner.address, 0, 1, [])
  ...
  describe("Function mintBatch", () => {
    it("Mint a batch of 3 random tokens", async () => {
      ...
      const prevBal = await cryptoChemical.balanceOfBatch(
        beneficiaryArray,
        ids
      );
      ...
      await cryptoChemical
        .connect(manager)
        .mintBatch(beneficiary.address, ids, amounts, []);
    ...
    it("Try mintBatch without being the manager", async () => {
      await expect(
        cryptoChemical.connect(owner).mintBatch(owner.address, [0], [1], [])
  ...
  describe("Function burn", () => {
    it("Function burn", async () => {
      ...
      await cryptoChemical
        .connect(manager)
        .burn(beneficiary.address, id, amount);
    ...
    it("Try burn without being the manager", async () => {
      ...
      await expect(
        cryptoChemical.connect(owner).burn(beneficiary.address, id, amount)
  ...
  describe("Function burnBatch", () => {
    it("Function burnBatch", async () => {
      ...
      await cryptoChemical
        .connect(manager)
        .burnBatch(beneficiary.address, ids, amounts);
